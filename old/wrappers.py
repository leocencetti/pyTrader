...
def fetchStocks(symbols, data_types, num_workers=10):
    ...
    for s in symbols:
        for d in data_types:
            taskQueue.put({"task_type": d, "key": key_generator(), "symbol": s})
...
def multiPlot(data, attribute, addAll=True):
    ...
    for column in data.keys():
        fig.add_trace(
            go.Scatter(x=data[column].index, y=data[column][attribute], name=column)
        )
    ...
    button_all = dict(
        ...
        method="restyle",
        args=[
            {"visible": [True for _ in data.keys()], "title": "All", "showlegend": True}
        ],
    )
    ...
    def create_layout_button(col):
        return dict(
            ...
            args=[
                {
                    "visible": [k == col for k in data.keys()],
                    "title": col,
                    "showlegend": True,
                }
            ],
    ...
    fig.update_layout(
        updatemenus=[
            dict(
                ...
                + [create_layout_button(col) for col in data.keys()],
            )
        ]
    )
    ...
    app = dash.Dash()
    app.layout = html.Div([dcc.Graph(style={"height": "90vh"}, figure=fig)])
