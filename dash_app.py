...
class DashApp:
    def __init__(self, data):
        ...
        self.app.callback(Output('subplot', 'figure'),
                          [Input('subtypeselector', 'value'), Input('symbolselector', 'value'),
                           Input('typeselector', 'value')])(self.update_subplots)
        self.app.callback(Output('subtypeselector', 'options'),
                          [Input('typeselector', 'value')])(self.update_subtype_dropdown)
            ...
            html.Div(className='row',
                     children=[
                                      ...
                                      html.Div(
                                          ...
                                          children=[
                                              ...
                                              dcc.Dropdown(id='symbolselector',
                                                           options=get_options(self.df['symbol'].unique()),
                                                           multi=True,
                                                           value=[get_default(self.df['symbol'].sort_values())],
                                                           className='symbolselector',
                                                           placeholder="Select a stock"
                                                           ),
                                              html.H5('Data Type'),
                                              dcc.Dropdown(id='typeselector',
                                                           options=get_options(self.df['type'].unique()),
                                                           value=['intraday'],
                                                           className='typeselector',
                                                           placeholder="Select a data type"
                                                           ),
                                              html.H5('Data Subtype'),
                                              dcc.Dropdown(id='subtypeselector',
                                                           options=get_options(self.df['subtype'].unique()),
                                                           value=['open'],
                         ...
                         html.Div(className='nine columns div-for-charts bg-grey',
                                  children=[
                                      dcc.Graph(id='subplot', config={'displayModeBar': True}, animate=False,
    ...
    def update_subplots(self, subtype_dropdown_value, symbol_dropdown_value, type_dropdown_value):
        ...
        for symbol, color in zip(symbol_dropdown_value, self.colors):
            ...
            figure.add_trace(go.Scatter(x=selected_data['date'],
