...
def get_intraday(symbol, key, interval="1min", outputsize="compact"):
    ...
    return AVReply(
        type=RequestType("{}_intraday".format(outputsize)),
...
def get_sma(symbol, key, interval="1min", time_period=200, series_type="close"):
    ...
    data.rename(columns={"SMA": "sma{}".format(time_period)}, inplace=True)
    return AVReply(
        type=RequestType("sma{}".format(time_period)),
        interval=Interval(interval),
        symbol=symbol,
...
def get_ema(symbol, key, interval="1min", time_period=200, series_type="close"):
    ...
    data.rename(columns={"EMA": "ema{}".format(time_period)}, inplace=True)
    return AVReply(
        type=RequestType("ema{}".format(time_period)),
        interval=Interval(interval),
        symbol=symbol,
        series=SeriesType(series_type),
        data=data,
